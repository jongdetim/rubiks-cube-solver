...
vi id(vi state)
{
	...
	//--- Phase 1: Edge orientations.    12 bits
	if (phase < 2)
	...
	// 8 keer corner orientatie (0, 1, 2) = ~11-16 bits

	// 12 keer edge posities (0, 1)(goed of slecht) = 12 bits

	// ! state[e] / 8 pakt alleen de posities van 4 edges
	// (state[e] / 8) is altijd 4 keer 1, en 8 keer 0
	if (phase < 3)
	{
...
vi scramble(vi state)
{
	for (int j = 0; j < 3; j++)
	{
		for (int i = 0; i < 18; i++)
			state = applyMove(i % (j + 7), state);
	}
	return state;
}
...
int main(int argc, char **argv)
{
	...
	currentState = scramble(currentState);
		...
		while (1)
		{
			...
			for (int move = 0; move < 18; move++)
			{
				if (applicableMoves[phase] & (1 << move))
				{
					...
					if (phase == 3)
					{
						for (std::vector<uint64_t>::const_iterator i = newId.begin(); i != newId.end(); ++i)
							std::cout << *i << ' ';
						cout << endl;
					}
					...
					if (newDir && newDir != oldDir)
					{
						...
						{
							cout << "UDFBLR"[algorithm[i] / 3] << algorithm[i] % 3 + 1 << endl;
